...
// Qrupları açmaq/bağlamaq (Accordion)
function toggleGroup(element) {
    // Valideyn element (nav-group) tapılır
    const group = element.parentElement;
    
    // Digər açıq qrupları bağlamaq istəsən bu kodu aktiv et:
    // document.querySelectorAll('.nav-group').forEach(g => {
    //     if (g !== group) g.classList.remove('active');
    // });

    // Kliklənən qrupu aç/bağla
    group.classList.toggle('active');
}

// Məzmunu göstərmək və seçimi aktiv etmək
function setActive(element, title, description) {
    // 1. Köhnə seçimi ləğv et
    ...
    // 3. Sağ tərəfdə məzmunu yenilə (Fade effekti ilə)
    const display = document.getElementById('content-display');
    
    // Yüngül keçid effekti
    ...
    setTimeout(() => {
        display.innerHTML = `
            ...
            <div class="content-body">
                <p>${description}</p>
                <br>
                <div style="background: #111; padding: 15px; border-left: 3px solid #ff4d4d; font-family: monospace; color: #aaa;">
                    $ root@oxsium:~# cat ${title.replace(/\s/g, '_').toLowerCase()}.txt<br>
                    > Accessing restricted database...<br>
